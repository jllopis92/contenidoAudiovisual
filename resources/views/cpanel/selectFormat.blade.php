...
@section('content')
	@if (!Auth::guest())
		@if (Auth::user()->tipo == "administrador")
			<h3 class="orangeAndBoldText" style="margin-bottom: 30px; padding-left: 15px;">Administrar Formatos</h3>
			<div class="col-md-10">
				<table class="table" data-filtering="true" data-paging="true" data-sorting="true">
					...
					<tbody>
						@foreach($formats as $format)
							<tr>
								<td>{{$format->name}}</td>
								<td data-toggle="modal" data-target="#formatModal" data-name="{!! $format->name!!}" data-id="{!!$format->id!!}" class="openform">
			...
			<div class="modal fade" id="formatModal" role="dialog">
...
@section('page-js-script')
	...
	<script type="text/javascript">
		...
		j(document).on("click", ".openform", function () {
			...
			document.getElementById("movieTittle").innerHTML = "¿Esta seguro de eliminar el Formato " + name + "?";
			//setSubmits();
		});

		j('#formatModal').on('hidden.bs.modal', function () {
			j("#state").empty();
		})
	</script>
@stop