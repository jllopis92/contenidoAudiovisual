				...
				<table class="table" data-filtering="true" data-paging="true" data-sorting="true">
					...
					<tbody>
						@foreach($types as $type)
							<tr>
								<td>{{$type->name}}</td>
								<td data-toggle="modal" data-target="#typeModal" data-name="{!! $type->name!!}" data-id="{!!$type->id!!}" class="openform">
...
@section('page-js-script')
	...
	<script type="text/javascript">
		...
		j(document).on("click", ".openform", function () {
			...
			document.getElementById("movieTittle").innerHTML = "¿Esta seguro de eliminar el Tipo de video " + name + "?";
			//setSubmits();
		});

		j('#typeModal').on('hidden.bs.modal', function () {
			j("#state").empty();
		})
	</script>
@stop